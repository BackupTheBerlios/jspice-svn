...
public abstract class FtpTools {
	...
	public static final String folderName( final String path, final String name ) {
		return path + name + StaticConf.VFOLDER_TERMINATOR;
	}
	...
	public static final String getName( final String path ) {
		final int n = path.lastIndexOf( StaticConf.VFOLDER_TERMINATOR );
		if ( n < 0 ) {
			return path;
		} else if ( n == path.length() - 1 ) {
			final int k = path.lastIndexOf( StaticConf.VFOLDER_TERMINATOR, n - 1 );
			if ( k < 0 ) {
	...
	public static final String getParentPath( final String path ) {
		final int n = path.indexOf( StaticConf.VFOLDER_TERMINATOR );
		if ( n < 0 ) {
			return "";
		} else {
			return path.substring( 0, n );
